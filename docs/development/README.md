...
| [pitfalls.md](pitfalls.md) | 踩坑记录：claude-code 一期与 codebuddy-code 二期真实踩过的坑与正确写法（含两者同名机制语义相反的对照） | 实现中报错先查这里；遇到新坑时补充 |

## 阅读路径

1. 先看仓库根 `README.md` 的总体架构与 `AGENTS.md` 的 Plugin Conventions（布局与命名规范）。
...
- 每次完成一个新工具的适配后，把该工具特有的决策与坑补充进对应文档，保持"最新经验在文档里"。
