...
describe('CodexSettingsLoader', () => {
  ...
  it('merges hooks from hooks.json and inline TOML across layers, deduplicating identical handlers', async () => {
    const files = new Map<string, string>([
      [
        fx('home', 'u', '.codex', 'hooks.json'),
        JSON.stringify({ hooks: { SessionStart: [{ matcher: 'startup', hooks: [sessionHook] }] } }),
      ],
      [
        fx('home', 'u', '.codex', 'config.toml'),
  ...
  it('disables hooks via [features].hooks = false in the most specific layer', async () => {
    const files = new Map<string, string>([
      [fx('home', 'u', '.codex', 'hooks.json'), JSON.stringify({ hooks: { Stop: [{ hooks: [{ type: 'command', command: 'x' }] }] } })],
      [fx('proj', '.codex', 'config.toml'), '[features]\nhooks = false\n'],
    ])
    const loaded = await makeLoader(files).load(fx('proj'))
  ...
  it('collects disabled skill paths from [[skills.config]] entries, resolving relative paths', async () => {
    const files = new Map<string, string>([
      [
        fx('home', 'u', '.codex', 'config.toml'),
        `[[skills.config]]\npath = '${fx('home', 'u', '.agents', 'skills', 'off-skill')}'\nenabled = false\n`,
      ],
      [
        fx('proj', '.codex', 'config.toml'),
    ...
    const loaded = await makeLoader(files).load(fx('proj'))
    expect(loaded.skillDisabledPaths).toContain(fx('home', 'u', '.agents', 'skills', 'off-skill'))
    expect(loaded.skillDisabledPaths).toContain(fx('proj', '.codex', '.agents', 'skills', 'proj-off'))
    expect(loaded.skillDisabledPaths).not.toContain(fx('proj', '.codex', '.agents', 'skills', 'on-skill'))
  })

  it('reads project_doc_* keys from the most specific layer with Codex defaults', async () => {
    const plain = await makeLoader(new Map()).load(fx('proj'))
  ...
  it('ignores invalid TOML and JSON files with a warning', async () => {
    const files = new Map<string, string>([
      ...
      [fx('proj', '.codex', 'hooks.json'), '{broken'],
    ])
    const loaded = await makeLoader(files).load(fx('proj'))
