...
const config = { userCodebuddyDir: fx('home', 'u', '.codebuddy'), maxBytes: 32_768 }
...
describe('collectMemorySections', () => {
  it('collects user and project memory files plus local memory in order', async () => {
    const files = new Map<string, string>([
      [fx('home', 'u', '.codebuddy', 'CODEBUDDY.md'), 'user memory'],
      [fx('proj', 'CODEBUDDY.md'), 'project root memory'],
      [fx('proj', 'CODEBUDDY.local.md'), 'local memory'],
    ])
    const sections = await collect(files)
    expect(sections.map((section) => section.label)).toEqual([
      fx('home', 'u', '.codebuddy', 'CODEBUDDY.md'),
      'CODEBUDDY.md',
      'CODEBUDDY.local.md',
    ])
  ...
  it('reads .codebuddy/CODEBUDDY.md and deduplicates identical content', async () => {
    const files = new Map<string, string>([
      [fx('proj', 'CODEBUDDY.md'), 'same content'],
      [fx('proj', '.codebuddy', 'CODEBUDDY.md'), 'same content'],
    ])
    const sections = await collect(files)
  ...
  it('collects recursive always-apply rules and strips their frontmatter', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.codebuddy', 'rules', 'style.md'), '---\nalwaysApply: true\n---\nRule body.\n'],
      [fx('proj', '.codebuddy', 'rules', 'nested', 'deep.md'), 'Deep rule.\n'],
      [fx('home', 'u', '.codebuddy', 'rules', 'prefs.md'), '---\nenabled: true\n---\nUser prefs.\n'],
    ])
    const sections = await collect(files)
    const labels = sections.map((section) => section.label)
    expect(labels).toContain(fx('home', 'u', '.codebuddy', 'rules', 'prefs.md'))
    expect(labels).toContain(fx('proj', '.codebuddy', 'rules', 'style.md'))
    expect(labels).toContain(fx('proj', '.codebuddy', 'rules', 'nested', 'deep.md'))
  ...
  it('skips disabled and conditional rules', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.codebuddy', 'rules', 'always.md'), 'Always.\n'],
      [fx('proj', '.codebuddy', 'rules', 'off.md'), '---\nenabled: false\n---\nOff.\n'],
      [fx('proj', '.codebuddy', 'rules', 'conditional.md'), '---\nalwaysApply: false\npaths: src/**/*.ts\n---\nConditional.\n'],
    ])
    const sections = await collect(files)
    expect(sections.map((section) => section.label)).toEqual([fx('proj', '.codebuddy', 'rules', 'always.md')])
  })
  ...
  it('loads a malformed-frontmatter rule as whole text (fail open)', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.codebuddy', 'rules', 'broken.md'), '---\nname: [unclosed\n---\nBroken frontmatter.\n'],
    ])
    const sections = await collect(files)
    expect(sections).toHaveLength(1)
