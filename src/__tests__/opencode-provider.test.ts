...
function makeProvider(files: Map<string, string>): OpencodeSkillProvider {
  ...
  const settings = new OpencodeSettingsLoader(silent, fs, { userOpencodeDir: fx('home', 'u', '.config', 'opencode') })
...
describe('OpencodeSkillProvider.list', () => {
  it('discovers project and user skills and commands', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.opencode', 'skills', 'deploy', 'SKILL.md'), '---\nname: deploy\ndescription: Deploy the app\n---\nDeploy body.\n'],
      [fx('proj', '.opencode', 'commands', 'lint.md'), '---\ndescription: Lint the repo\n---\nRun lint.\n'],
      [
        fx('home', 'u', '.config', 'opencode', 'skills', 'personal', 'SKILL.md'),
        '---\nname: personal\ndescription: Personal skill\n---\nBody.\n',
      ],
      [fx('home', 'u', '.config', 'opencode', 'commands', 'notes.md'), '# Notes\n\nTake notes.\n'],
    ])
  ...
  it('assigns ranks: project beats user, skills beat commands (opencode precedence)', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.opencode', 'skills', 'shared', 'SKILL.md'), '---\nname: shared\ndescription: project skill\n---\na\n'],
      [fx('proj', '.opencode', 'commands', 'shared.md'), '---\ndescription: project command\n---\nb\n'],
      [fx('home', 'u', '.config', 'opencode', 'skills', 'shared', 'SKILL.md'), '---\nname: shared\ndescription: user skill\n---\nc\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips skills whose name does not match the directory name', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.opencode', 'skills', 'mismatch', 'SKILL.md'), '---\nname: other\ndescription: x\n---\na\n'],
      [fx('proj', '.opencode', 'skills', 'good', 'SKILL.md'), '---\nname: good\ndescription: y\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips skills without a description (required by opencode)', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.opencode', 'skills', 'no-desc', 'SKILL.md'), '---\nname: no-desc\n---\na\n'],
      [fx('proj', '.opencode', 'skills', 'with-desc', 'SKILL.md'), '---\nname: with-desc\ndescription: has one\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips skill names that are not valid opencode names', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.opencode', 'skills', 'git_release', 'SKILL.md'), '---\nname: git_release\ndescription: underscore\n---\na\n'],
      [fx('proj', '.opencode', 'skills', 'git-release', 'SKILL.md'), '---\nname: git-release\ndescription: hyphen\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('ignores flat md files and nested directories in commands', async () => {
    const files = new Map<string, string>([
      [fx('proj', '.opencode', 'commands', 'top.md'), '---\ndescription: top\n---\na\n'],
      [fx('proj', '.opencode', 'commands', 'nested', 'inner.md'), '---\ndescription: inner\n---\nb\n'],
      [fx('proj', '.opencode', 'commands', 'notmd.txt'), 'x'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips JSON-configured commands whose name is not kebab-case', async () => {
    ...
    const files = new Map<string, string>([
      [fx('proj', 'opencode.json'), JSON.stringify({ command: { 'Not-Kebab': { template: 'bad' }, good: { template: 'ok' } } })],
    ])
    const fs = new TreeFs(files)
    const settings = new OpencodeSettingsLoader(logger, fs, { userOpencodeDir: fx('home', 'u', '.config', 'opencode') })
  ...
  it('derives the description from the first paragraph when a command omits it', async () => {
    const files = new Map<string, string>([[fx('proj', '.opencode', 'commands', 'plain.md'), '# Title\n\nDerived description.\n']])
    const result = await makeProvider(files).list(options)
...
describe('OpencodeSkillProvider.get', () => {
  it('loads the body of a skill bundle', async () => {
    ...
    expect(definition?.resourceBase).toEqual({ kind: 'directory', path: fx('proj', '.opencode', 'skills', 'deploy') })
  })

  it('loads the template of a JSON-configured command', async () => {
    const files = new Map<string, string>([
      [fx('proj', 'opencode.json'), JSON.stringify({ command: { test: { template: 'Run tests', description: 'Tests' } } })],
    ])
    const result = await makeProvider(files).list(options)
    const candidate = result.candidates[0]!
    const definition = await makeProvider(files).get(candidate, options)
  ...
  it('returns undefined when a JSON command is removed from config', async () => {
    const files = new Map<string, string>([[fx('proj', 'opencode.json'), JSON.stringify({ command: { test: { template: 'Run tests' } } })]])
    const provider = makeProvider(files)
    const result = await provider.list(options)
    files.set(fx('proj', 'opencode.json'), JSON.stringify({ command: {} }))
