...
function makeProvider(files: Map<string, string>): CodexSkillProvider {
  ...
  const settings = new CodexSettingsLoader(silent, fs, { userCodexDir: '/home/u/.codex' })
...
const options: SkillLookupOptions = { cwd: '/proj/sub' }

describe('CodexSkillProvider.list', () => {
  it('discovers project, user, and system skill bundles', async () => {
    const files = new Map<string, string>([
      ['/proj/.git/HEAD', 'x'],
      ['/proj/sub/.agents/skills/deploy/SKILL.md', '---\nname: deploy\ndescription: Deploy\n---\nBody.\n'],
      ['/home/u/.agents/skills/personal/SKILL.md', '---\nname: personal\ndescription: Personal\n---\nBody.\n'],
      ['/etc/codex/skills/admin-skill/SKILL.md', '---\nname: admin-skill\ndescription: Admin\n---\nBody.\n'],
    ])
  ...
  it('scans .agents/skills from the cwd up to the repository root, closest first', async () => {
    const files = new Map<string, string>([
      ['/proj/.git/HEAD', 'x'],
      ['/proj/sub/.agents/skills/inner/SKILL.md', '---\nname: inner\ndescription: Inner\n---\na\n'],
      ['/proj/.agents/skills/root/SKILL.md', '---\nname: root\ndescription: Root\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('ranks project skills below user and system skills (DSH lower-rank wins)', async () => {
    const files = new Map<string, string>([
      ['/proj/.git/HEAD', 'x'],
      ['/proj/sub/.agents/skills/shared/SKILL.md', '---\nname: shared\ndescription: project\n---\na\n'],
      ['/home/u/.agents/skills/shared/SKILL.md', '---\nname: shared\ndescription: user\n---\nb\n'],
      ['/etc/codex/skills/shared/SKILL.md', '---\nname: shared\ndescription: system\n---\nc\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips skills disabled via [[skills.config]]', async () => {
    const files = new Map<string, string>([
      ['/proj/.git/HEAD', 'x'],
      ['/proj/sub/.agents/skills/off/SKILL.md', '---\nname: off\ndescription: Off\n---\na\n'],
      ['/proj/sub/.agents/skills/on/SKILL.md', '---\nname: on\ndescription: On\n---\nb\n'],
      ['/proj/.codex/config.toml', '[[skills.config]]\npath = "/proj/sub/.agents/skills/off"\nenabled = false\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips invalid names and skills missing required frontmatter', async () => {
    const files = new Map<string, string>([
      ['/proj/.git/HEAD', 'x'],
      ['/proj/sub/.agents/skills/camelCase/SKILL.md', '---\nname: camelCase\ndescription: x\n---\na\n'],
      ['/proj/sub/.agents/skills/no-desc/SKILL.md', '---\nname: no-desc\n---\nb\n'],
      ['/proj/sub/.agents/skills/good/SKILL.md', '---\nname: good\ndescription: y\n---\nc\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('ignores flat md files in skills directories', async () => {
    const files = new Map<string, string>([
      ['/proj/.git/HEAD', 'x'],
      ['/proj/sub/.agents/skills/flat.md', '---\nname: flat\ndescription: x\n---\na\n'],
      ['/proj/sub/.agents/skills/bundle/SKILL.md', '---\nname: bundle\ndescription: y\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
...
describe('CodexSkillProvider.get', () => {
  it('loads the body of a skill bundle', async () => {
    ...
    expect(definition?.resourceBase).toEqual({ kind: 'directory', path: '/proj/sub/.agents/skills/deploy' })
  })
})
