...
function makeLoader(files: Map<string, string>): SettingsLoader {
  return new SettingsLoader(silent, new TreeFs(files), { userClaudeDir: '/home/u/.claude' })
}
...
describe('collectMemorySections', () => {
  ...
  it('collects user, project, hierarchy, and local files in order', async () => {
    const files = new Map<string, string>([
      ...
      ['/repo/CLAUDE.md', 'repo memory'],
      ['/repo/.claude/CLAUDE.md', 'project memory'],
      ['/repo/CLAUDE.local.md', 'repo local memory'],
      ['/repo/apps/CLAUDE.md', 'api memory'],
      ['/repo/apps/api/CLAUDE.local.md', 'local memory'],
    ])
    const sections = await collectMemorySections('/repo/apps/api', silent, new TreeFs(files), config)
  ...
  it('skips hierarchy CLAUDE.md identical to the cwd-level file the core loads', async () => {
    const files = new Map<string, string>([
      ['/repo/CLAUDE.md', 'same content'],
      ['/repo/apps/api/CLAUDE.md', 'same content'],
    ])
    const sections = await collectMemorySections('/repo/apps/api', silent, new TreeFs(files), config)
    expect(sections.map((section) => section.label)).toEqual([])
  })
  ...
  it('loads memory files from permissions.additionalDirectories', async () => {
    const files = new Map<string, string>([
      ['/other/CLAUDE.md', 'other memory'],
      ['/repo/.claude/settings.json', JSON.stringify({ permissions: { additionalDirectories: ['../other'] } })],
    ])
    const sections = await collectMemorySections('/repo', silent, new TreeFs(files), { ...config, settingsLoader: makeLoader(files) })
  ...
  it('returns only the user section without a workspace', async () => {
    const files = new Map<string, string>([['/home/u/.claude/CLAUDE.md', 'user memory']])
