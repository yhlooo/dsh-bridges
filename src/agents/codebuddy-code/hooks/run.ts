/**
 * CodeBuddy Code hook execution: matcher filtering, command and HTTP
 * handlers, exit-code/JSON output capture.
 *
 * Command hooks replicate CodeBuddy Code's contract: JSON input on stdin,
 * exit code 2 as the blocking signal, and — on block — the message priority
 * stdout (JSON `reason`/`stopReason` or plain text) over stderr. Other
 * non-zero exit codes are non-blocking errors. `command` plus `args` spawns
 * in exec form (no shell); a bare `command` runs through the system shell.
 * HTTP hooks send the same JSON with `method` (`POST` by default, `PUT`,
 * `PATCH`) and read the same output vocabulary from the response body.
 *
 * `prompt` and `agent` handler types need an LLM subagent and are not bridged
 * yet; they are dropped during settings normalization (see the README
 * limitations).
 * @module dsh-bridges/agents/codebuddy-code/hooks/run
 */
import { spawn, type ChildProcess } from 'node:child_process'
import type { BridgeLogger } from '../../../util.js'
import { isPlainObject } from '../../../util.js'
import { matchIf, matchMatcher } from './matcher.js'
...
export interface HookRun {
  ...
  matchedValue?: string
  /** The JSON payload handed to handlers on stdin / as the request body. */
  input: Record<string, unknown>
  /** Working directory handlers spawn in. */
  cwd: string
  /** Absolute project root substituted into `${CODEBUDDY_PROJECT_DIR}`. */
  projectDir: string
...
function spawnCommand(handler: CommandHook, run: HookRun): ChildProcess {
  ...
  const env = { ...process.env, CODEBUDDY_PROJECT_DIR: run.projectDir, ...run.env }
...
function substituteProjectDir(value: string, projectDir: string): string {
  return value.replace(/\$\{CODEBUDDY_PROJECT_DIR\}/g, projectDir)
}
...
async function runHttpHook(handler: HttpHook, run: HookRun, logger: BridgeLogger, base: HookOutcome): Promise<HookOutcome> {
  ...
  try {
    ...
    logger.debug(`codebuddy-code: ${run.event} HTTP hook ${handler.url} settled`)
  }
}

// ── output parsing ───────────────────────────────────────────────────────────

/** Split settled stdout into parsed JSON output vs plain text, per CodeBuddy Code rules. */
...
/**
 * The message an exit-2 hook blocks with, following CodeBuddy Code's
 * priority: stdout JSON `reason`/`stopReason`, then plain stdout, then
 * stderr as fallback.
 */
export function hookBlockMessage(outcome: HookOutcome): string | undefined {
  const jsonReason = typeof outcome.output?.reason === 'string' ? outcome.output.reason : undefined
  const stopReason = typeof outcome.output?.stopReason === 'string' ? outcome.output.stopReason : undefined
  if (jsonReason !== undefined && jsonReason.trim() !== '') return jsonReason
  if (stopReason !== undefined && stopReason.trim() !== '') return stopReason
  if (outcome.plainText !== null && outcome.plainText.trim() !== '') return outcome.plainText
  if (outcome.stderr.trim() !== '') return outcome.stderr
  return undefined
}
