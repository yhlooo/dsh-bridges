/**
 * CodeBuddy Code `SKILL.md` / command-file frontmatter parsing and field
 * mapping.
 *
 * The frontmatter vocabulary overlaps Claude Code's: `name`, `description`,
 * `disable-model-invocation`, `user-invocable`. `when_to_use` is not listed in
 * the CodeBuddy Code docs but is tolerated (merged into the description like
 * Claude Code does) so skills written for Claude Code keep working. Fields
 * without a DSH equivalent (`allowed-tools`, `context: fork`, `agent`,
 * `model`, skill frontmatter `hooks`) are ignored by discovery.
 * @module dsh-bridges/agents/codebuddy-code/skills/parse
 */
...
export interface ParsedFrontmatter {
  /** Display name (CodeBuddy Code uses the directory/file name for invocation). */
  name?: string
  /** Routing description, as written. */
  description?: string
  /** Additional routing guidance (Claude Code compat), as written. */
  whenToUse?: string
...
/**
 * Derive a routing description from the body the way CodeBuddy Code does when
 * a file omits `description`: the first non-empty paragraph after the title.
 */
