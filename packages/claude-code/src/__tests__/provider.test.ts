...
function makeProvider(files: Map<string, string>): ClaudeSkillProvider {
  return new ClaudeSkillProvider(silent, new TreeFs(files), { userClaudeDir: '/home/u/.claude', watch: false }, () => {})
}
...
describe('ClaudeSkillProvider.list', () => {
  it('discovers project and user skill bundles and commands', async () => {
    const files = new Map<string, string>([
      ['/home/u/.claude/skills/personal/SKILL.md', '---\ndescription: Personal skill\n---\nBody.\n'],
      ['/proj/.claude/skills/deploy/SKILL.md', '---\ndescription: Deploy\n---\nDeploy body.\n'],
      ['/proj/.claude/commands/lint.md', '---\ndescription: Lint the repo\n---\nRun lint.\n'],
      ['/home/u/.claude/commands/notes.md', '# Notes\n\nTake notes.\n'],
    ])
  ...
  it('skips the reserved synced directory in the user skills root', async () => {
    const files = new Map<string, string>([
      ['/home/u/.claude/skills/synced/SKILL.md', '---\ndescription: synced\n---\na\n'],
      ['/home/u/.claude/skills/real/SKILL.md', '---\ndescription: real\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('skips names that are not kebab-case', async () => {
    const files = new Map<string, string>([
      ['/proj/.claude/skills/camelCase/SKILL.md', '---\ndescription: nope\n---\na\n'],
      ['/proj/.claude/skills/good-name/SKILL.md', '---\ndescription: yes\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('derives the description from when_to_use and the first paragraph', async () => {
    const files = new Map<string, string>([
      ['/proj/.claude/skills/with-when/SKILL.md', '---\ndescription: Base\nwhen_to_use: When asked\n---\nBody.\n'],
      ['/proj/.claude/skills/no-desc/SKILL.md', '---\nname: display\n---\n# Title\n\nFell back paragraph.\n'],
    ])
    const result = await makeProvider(files).list(options)
  ...
  it('maps invocation policy from the frontmatter', async () => {
    const files = new Map<string, string>([
      ['/proj/.claude/skills/task/SKILL.md', '---\ndisable-model-invocation: true\n---\na\n'],
      ['/proj/.claude/skills/hidden/SKILL.md', '---\nuser-invocable: false\n---\nb\n'],
    ])
    const result = await makeProvider(files).list(options)
...
describe('ClaudeSkillProvider.get', () => {
  it('loads the current body and resource base for a bundle', async () => {
    const files = new Map<string, string>([
      ['/proj/.claude/skills/deploy/SKILL.md', '---\ndescription: Deploy\n---\nStep one.\n'],
      ['/proj/.claude/skills/deploy/template.md', 'x'],
    ])
    ...
    expect(definition?.resourceBase).toEqual({ kind: 'directory', path: '/proj/.claude/skills/deploy' })
  })
